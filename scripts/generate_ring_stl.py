#!/usr/bin/env python3
"""
Generate STL file for a ring geometry for OpenFOAM simulations.

Parameters:
- D: Outer diameter (m)
- t: Thickness (m)
- w: Width (axial length) (m)
- center: Center position (x, y, z)
- resolution: Number of facets around circumference
"""
...
def generate_ring_stl(D, t, w, center=(0, 0, 0), resolution=60, filename="ring.stl"):
    ...
    The ring is modeled as a hollow cylinder (tube) with:
    - Outer radius: R = D/2
    - Inner radius: r = (D - 2*t)/2
    - Height: w (in z-direction)
    ...
    # Write STL file
    write_stl(vertices, faces, filename)
...
def main():
    """Main function with example parameters."""
    # Example parameters (base case)
    D = 0.1      # Outer diameter: 10 cm
    t = 0.005    # Thickness: 5 mm
    w = 0.01     # Width: 1 cm

    # Center position (above water surface)
    center = (0, 0, 0.55)  # 5 cm above water surface (z=0.5)

    # Generate STL
    filename = "ring.stl"
    generate_ring_stl(D, t, w, center, resolution=60, filename=filename)

    print("\nTo use different parameters, modify the main() function or call:")
    print("  generate_ring_stl(D, t, w, center, resolution=60, filename='ring.stl')")
